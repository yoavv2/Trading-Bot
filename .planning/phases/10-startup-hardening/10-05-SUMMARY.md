...
**Pre-existing, unrelated intermittent test-teardown flake — reproduced against pre-10-05 code, not just inferred.** Full-suite runs occasionally (not on every run) report 1-3 `ERROR`s at teardown of `migrated_access_db`-style fixtures (`test_market_data_access.py`, `test_market_data_ingestion.py`, `test_portfolio_service.py`, `test_risk_pipeline.py`, others) with `psycopg.errors.InsufficientPrivilege: must be a superuser to terminate superuser process` during their `pg_terminate_backend` cleanup step. Rather than relying only on "passes in isolation + varies run-to-run" as evidence, checked out the commit immediately before this plan's own first commit (`24e58c3`, before both 10-04 and 10-05) into an isolated `git worktree` and ran `python -m pytest -q` three times against that untouched code — the identical flake reproduced on every run (different test files each time). This rules out both 10-04's and this plan's changes as the cause. The most plausible origin is 10-03 (earlier in this same phase), which rewrote the engine/session-cache lifecycle in `db/session.py` that every affected fixture's DB access goes through; this plan's own gate connects as the non-superuser configured DB user, so it structurally cannot be the superuser session the teardown fails to terminate. Logged in `deferred-items.md` with the reproduction method for a follow-up (out of this plan's `files_modified` scope — the pattern is duplicated across 6+ unrelated test files).

**`api/app.py`'s `main()` still calls the ungated `load_settings()`.** Only the FastAPI `lifespan` (the plan's named target) was wired to the gate; `main()` — the actual `uvicorn.run` entrypoint used when running `python -m trading_platform.api` directly — still calls `load_settings()` first to read `api.host`/`api.port`. An invalid config would surface as a raw pydantic traceback from `main()` before the (also-invalid-config-detecting) `lifespan` gate ever gets a chance to run inside uvicorn's startup sequence. This is within the plan's literal scope (the action text names only `lifespan`), and the gate still fires correctly on every real boot via `TestClient`/uvicorn's lifespan protocol, so it is not a functional gap in CFG-04/CFG-06 as tested — but a follow-up could route `main()` through `enforce_startup_config` too (or simply let `lifespan`'s failure be the sole enforcement point and accept `main()`'s pre-lifespan `load_settings()` call as informational-only) for a fully consistent single-message failure mode on direct `python -m` invocation.
